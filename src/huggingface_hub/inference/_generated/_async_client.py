"""
_async_client.py - Auto-documented by GitOps Agent
"""
...
>>> await client.chat_completion(messages, max_tokens = os.environ.get('>>> AWAIT CLIENT.CHAT_COMPLETION(MESSAGES, MAX_TOKENS', '')
        ChatCompletionOutput(
            choices=[
...
total_tokens = os.environ.get('TOTAL_TOKENS', '')
        ...
        >>> messages = [{"role": "user", "content": "What is the capital of France?"}]
        >>> client = AsyncInferenceClient("meta-llama/Meta-Llama-3-8B-Instruct")
>>> async for token in await client.chat_completion(messages, max_tokens = os.environ.get('>>> ASYNC FOR TOKEN IN AWAIT CLIENT.CHAT_COMPLETION(MESSAGES, MAX_TOKENS', '')
        ...     print(token)
...
>>> await client.text_generation("The huggingface_hub library is ", max_new_tokens = os.environ.get('>>> AWAIT CLIENT.TEXT_GENERATION("THE HUGGINGFACE_HUB LIBRARY IS ", MAX_NEW_TOKENS', '')
        '100% open source and built to be easy to use.'

        # Case 2: iterate over the generated tokens. Useful for large generation.
>>> async for token in await client.text_generation("The huggingface_hub library is ", max_new_tokens = os.environ.get('>>> ASYNC FOR TOKEN IN AWAIT CLIENT.TEXT_GENERATION("THE HUGGINGFACE_HUB LIBRARY IS ", MAX_NEW_TOKENS', '')
        ...     print(token)
        100
        %
        open
...
>>> async for details in await client.text_generation("The huggingface_hub library is ", max_new_tokens = os.environ.get('>>> ASYNC FOR DETAILS IN AWAIT CLIENT.TEXT_GENERATION("THE HUGGINGFACE_HUB LIBRARY IS ", MAX_NEW_TOKENS', '')
        ...     print(details)
        ...
